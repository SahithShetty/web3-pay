...
const Recipients = () => {
  ...
  useEffect(() => {
    async function getData() {
      const recipients = await App.paypalContract.filters.recipients(App.address)
  ...
  return (
    <div className='flex flex-col items-center justify-center py-4 px-1 text-white'>
      ...
      <input onChange={(e) => setRecipientName(e.target.value)}
        value={recipientName} className="mt-2 w-3/4 p-3 bg-black border-2
       border-blue-900 border-opacity-60 bg-opacity-70 outline-none rounded-lg"
      ...
      <div className='flex flex-col items-center justify-center mt-4 w-full'>
        ...
        {data.map((e) => {
          return (
            <div onClick={() => setRecipient(e.args.recipient, e.args.recipientName)} className={`bg-black cursor-pointer rounded-lg bg-opacity-60 border-2 border-blue-900
        border-opacity-80 w-3/4 mt-2`}>
              <div className="flex w-full items-center justify-center rounded-t-lg">
                <div className="w-full py-2 px-2">
                  ...
                  <p className="text-xl font-mono"> Address: {e.args.recipient}</p>
                </div>
              </div>
            </div>
          )
